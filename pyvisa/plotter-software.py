...
class App:
    ...
    def _do_or_tear(self):
        ...
        if self._live_mode and self._ic is not None:
            ...
            self._recompute_batch()
            t_now = self.times[tear_idx] if self.times is not None else tear_idx * 0.075
            t_str = f'{t_now:.1f}'
            self._log(f'TARE (overrange stable) @ {c_val:.3f} mV — full state reset','tear',t_str)
    ...
    def _play(self):
        ...
        self._log(f'computing {len(filtered):,} samples  (warmup min {warmup*0.075:.0f} s, SD < {WARMUP_SD_THRESH} gate)','info')
    ...
    def _start_live(self,path):
        ...
        self._clear_log()
        warmup=int(self.params.get('warmup_samples',BASELINE_WARMUP_SAMPLES))
        self._log(f'Live — warmup: {warmup} samples ({warmup*0.075:.0f} s min), SD < {WARMUP_SD_THRESH} gate','warn')
